...
class Matrix {
    ...
    x;
    static traversal = {
        abscissa: (magnitude, offset) => (entry, index) => Array.of(...Array(magnitude[1] - 1).fill([0, 1]), [1, 1 - magnitude[1]]),
        ordinate: (magnitude, offset) => (entry, index) => Array.of(...Array(magnitude[0] - 1).fill([1, 0]), [1 - magnitude[0], 1]),
    };
    static factory(source) {
        switch (true) {
            ...
            case Matrix.isIter(source): return (y, x) => source.next([y, x]).value;
